...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT * FROM admins WHERE admin_email  = :adminName";

    $statement = $conn->prepare($query);
    $statement->bindParam(':adminName', $name);
    $statement->execute();

    if ($statement->rowCount() === 1) {
        $row = $statement->fetch(PDO::FETCH_ASSOC);
        $_SESSION['loggedInAdmin'] = $row['id'];
    ...
    <main class="max-w-xl lg:max-w-7xl px-10 mx-auto">
        <div class="flex items-center justify-center min-h-screen">
            <div class="px-10 py-3 pt-14 text-left bg-secondarylightmode dark:bg-secondarydarkmode shadow-lg">
                ...
                <h3 class="text-xl font-bold text-center text-black dark:text-white mt-6">Admin login</h3>
                    ...
                    <div class="mt-4">
                        ...
                        <?php
                            if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($_POST['name'])) {
                                echo '<span class="text-xs tracking-wide text-red-600">Name field is required*</span>';
                            }
                        ?>
