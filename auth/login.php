    ...
    if (!empty($email) && !empty($password)) {
        ...
        if ($statement->rowCount() === 1) {
            ...
            if (password_verify($password, $row['user_password'])) {
                setcookie("loggedInUser", $row['id'], time()+30*24*60*60, "/");
    ...
    <main class="max-w-xl lg:max-w-7xl px-10 mx-auto">
        <div class="flex items-center justify-center min-h-screen">
            <div class="px-10 py-3 pt-14 text-left bg-secondarylightmode dark:bg-secondarydarkmode shadow-lg">
                ...
                <h3 class="text-xl font-bold text-center text-black dark:text-white mt-5">Login to your account</h3>
                <p class="text-red-600 text-center mt-3">
                    <?php global $error; 
                    echo $error;?>
                </p>
                <form method="POST">
                    <div class="mt-4">
                        <div>
                            <label class="block text-black dark:text-white" for="email">Email:</label>
                        ...
                        <div class="mt-4 flex flex-col">
                            <label class="block text-black dark:text-white" for="password">Password:</label>
                            <input id="password" name="password" type="password" placeholder="Password*"
                                class="w-full text-black pl-4 sm:pr-48 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600">
                        </div>
                        <?php
                            if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($_POST['password'])) {
                                echo '<span class="text-xs tracking-wide text-red-600">Password field is required*</span>';
                            }
                        ?>
                        <div class="flex flex-col md:flex-row items-baseline justify-between">
                            <button
                                class="px-6 py-2 mt-5 text-white bg-[#EA0505] mx-auto sm:mx-0 rounded-lg hover:bg-red-600">Login</button>
                            <a href="#"
                                class="text-sm mt-4 sm:mt-0 mx-auto sm:mx-0 text-blue-600 hover:underline">Forgot
                                password?</a>
                        </div>
                        <div class="flex justify-center mt-8 mb-8">
                            <span class="text-black dark:text-white text-center">Not registered? <a
                                    class="text-[#EA0505] hover:text-red-500" href="register.php">Create an
                                    account<a></span>
                        </div>
                    </div>
                </form>
